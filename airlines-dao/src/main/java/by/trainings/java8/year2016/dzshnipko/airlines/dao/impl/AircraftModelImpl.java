...
@Repository
public class AircraftModelImpl extends AbstractDaoImpl<AircraftModel, Long, AircraftModelFilter>
		implements AircraftModelDAO {
	...
	@Override
	protected void handleFilterParameters(AircraftModelFilter filter, CriteriaBuilder cb, CriteriaQuery<?> cq,
			Root<AircraftModel> from) {
		...
		if (aircraftModelFilter.getMaxTransportedCargoMax() != null) {
			predicates.add(cb.lt(from.get(AircraftModel_.maxTransportedCargo),
					aircraftModelFilter.getMaxTransportedCargoMax()));
	...
	protected void fetchLazyInitilization(AircraftModelFilter filter, Root<AircraftModel> from) {
		if (filter.isFetchAircrafts()) {
			from.fetch(AircraftModel_.aircrafts, JoinType.LEFT);
		}
	}
